...
def ParseArgs():
    ...
    discover_parser.add_argument('-F --final-stage', dest='terminal_stage', choices=STAGES, default=STAGES[len(STAGES)-1], help='The stage %(prog)s should finish with (default %(default)s)')
    discover_parser.add_argument('queries', nargs='+', metavar='Query:QUERY TYPE:Approx Result Count', help='Search queries to use when scraping YouTube. You can also improve accuracy of the scraper by tagging your query type in the following form: Q:{Artist,Track,Album,*}:N. For example, RZA:Artist:10 would start a search for a maximum of 10 candidate tracks of the Artist, RZA. Big Boi:*:10 for any video from the query Big Boi.')
...
class MuseConfig(object):
    def __init__(self, parsed_args):
        ...
        config_path = os.path.join(self.home_dir, 'config.txt')
        from_cfg = _loadConfig(config_path)
    ...
    def _createSystemResources(self):
        _ensureDirCreation(self.home_dir)
        _ensureDirCreation(self.sessions_dir)
        _ensureDirCreation(self.cur_session_dir)
    
    @staticmethod
    def _ensureFileCreation(path):
